...
import { CRAFT_FLOOR, designBlock } from "./design";
...
- Layout: responsive grid (grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 for KPIs; charts usually xl:col-span-2). Generous whitespace; nothing cramped.
...
export function assemblePlan(
  ...
  const { domain, block } = buildEnrichment(datasets);
  const adaptNote =
    domain === "generic"
...
export function assemble(input: AssembleInput): { system_prompt: string; user_prompt: string } {
  const { datasets, userPrompt, currentCode, lastError, dataAccess, sqlDialect, plan } = input;
...
- Use the exact presentation form the user names (chart type, table, section, filter).`;
  ...
  if (lastError && currentCode) {
    // Known failure class: assigning to an immutable binding. The bare error
    // string ("x" is read-only) gives the model too little to go on, so spell
    // out the repair strategy explicitly.
    const readOnlyHint = /read[- ]?only|invalid assignment|assignment to constant/i.test(lastError)
      ? "Hint: this error means the code assigned to something immutable — an imported binding (rows, tables, query, selectFeature), a const, or a reserved global (message, status, name, event, location). Copy imports into fresh local variables before changing them (e.g. const data = [...rows]), rename any variable that shadows a reserved global, and never reassign imports or consts."
      : "";
    user_prompt = [
      "The current app threw this runtime error:", lastError,
      readOnlyHint,
    ...
    const detected = buildEnrichment(datasets).domain;
    const exemplar = exemplarBlock(detected);
    user_prompt = [
