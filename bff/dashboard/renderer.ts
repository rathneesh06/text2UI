...
const PLAN = ${planJson};
const COLORS = ["#4f46e5", "#818cf8", "#a5b4fc", "#64748b", "#94a3b8", "#c7d2fe"];
...
function Card(props) {
  return (
    <div className={widthClass(props.width) + " bg-white rounded-xl border border-slate-200 shadow-sm p-5"}>
      {props.title ? <div className="text-sm font-medium text-slate-700">{props.title}</div> : null}
...
function Kpi(props) {
  ...
  return (
    <div className={widthClass(w.width || "quarter") + " bg-white rounded-xl border border-slate-200 shadow-sm p-5 cursor-pointer"}
      ...
      {s.loading ? <div className="mt-2 h-8 w-24 animate-pulse rounded bg-slate-100" />
        ...
        : <div className="mt-1 text-3xl font-semibold tabular-nums text-slate-900">{fmt(value, (w.metric && w.metric.format) || w.format)}</div>}
    </div>
  );
}
    ...
    return (
      <ResponsiveContainer width="100%" height={240}>
        <Cmp data={data}>
          ...
          <XAxis dataKey="x" tick={{ fill: "#64748b", fontSize: 12 }} axisLine={false} tickLine={false} />
